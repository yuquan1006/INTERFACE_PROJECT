...
public class HttpMirrorThread implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            // We assume headers are in UTF_8
            ...
            while(positionOfBody <= 0 && ((length = in.read(buffer)) != -1)) {
                ...
                headers.append(new String(buffer, 0, length, UTF_8));
            ...
            // The headers are written using UTF_8 encoding
            out.write(("HTTP/1.0 "+responseStatusValue).getBytes(UTF_8)); //$NON-NLS-1$
            out.write(CRLF);
            out.write("Content-Type: text/plain".getBytes(UTF_8)); //$NON-NLS-1$
            out.write(CRLF);
            ...
            if (parameters.containsKey(REDIRECT)) {
                ...
                out.write(redirectLocation.getBytes(UTF_8));
                out.write(CRLF);
            }
            ...
            if (cookieHeaderValue != null) {
                out.write("Set-Cookie: ".getBytes(UTF_8));
                out.write(cookieHeaderValue.getBytes(UTF_8));
                out.write(CRLF);
            }
            out.write(CRLF);
            out.flush();
